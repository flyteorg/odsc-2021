...
A folder is the fundamental operating system primitive users might find themselves working with.

This example demonstrates how to use FlyteDirectory to represent a folder in Flyte.
"""
...
# %%
# This task downloads the two files mentioned above using non-Flyte libraries, and returns the path to the folder, in a FlyteDirectory object.
@task
...
# %%
# This task accepts the previously downloaded folder, and calls the rotate function above on each.
# Since the rotate function does the image manipulation in place, we just create a new FlyteDirectory object pointing to the same place.
@task
